...
function latestFullPath = getLatestFittingDateStr(basePath)
    ...
    for k = 1:length(items)
        ...
        if ~isempty(tokens)
            ...
            try
                % Parse date manually for MATLAB 2017 compatibility
                dateParts = strsplit(dateStr, '-');
                day = str2double(dateParts{1});
                month = str2double(dateParts{2});
                year = str2double(dateParts{3});
                ...
                % Get today's date
                today = datetime('now');
                today_date_only = datetime(year(today), month(today), day(today));
                
                if Dt ~= today_date_only  % Exclude today's date
                    validDates(end+1) = datenum(Dt);
