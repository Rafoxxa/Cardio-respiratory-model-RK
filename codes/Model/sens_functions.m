function out = sens_functions(mode, fun, setup)
    ...
    elseif strcmp(mode, 'loading')
        [sens_matrix, pars_to_sens] = load_sens(s);
        out = {sens_matrix, pars_to_sens};
    elseif strcmp(mode, 'p-space')
         out = show_p_space(s);     
    end    
    ...
    function sens_final_time_matrix = build_LSA_matrix(s, variables_of_interest, sensitivities, perturbed)
       ...
       for v = 1:num_variables
            ...
            if ismember(key, extra_vars)
                ...
                target_variable_idx = find(strcmp(init.keys, key)); % Cambio para containers.Map                
            end
    ...
    function [reduced_sens, reduced_pars] = filter_params_by_class(s)
        ...
        % En R2017, usar xlsread en lugar de readtable
        [~, ~, excelData] = xlsread('parameters_classified_ori.xlsx');  
        
        % Encontrar la columna "Class" y "Parameter"
        header_row = excelData(1, :);
        class_col = find(strcmp(header_row, 'Class'));
        param_col = find(strcmp(header_row, 'Parameter'));
        
        % Extraer datos (excluyendo header)
        classes = excelData(2:end, class_col);
        parameters = excelData(2:end, param_col);
        
        % Encontrar parámetros "Arbitrary"
        isArbitrary = strcmp(classes, 'Arbitrary');
        arbitraryParams = parameters(isArbitrary);   
    ...
    function pars = add_noise(pars, pars_free2move, noise)
        rng('shuffle');  % En lugar de rng("shuffle")
    ...
    x_vals = linspace(min(M_nonzero(:)), max(M_nonzero(:)), 200);  % Cambio para R2017
    ...
    for i = 1:num_datasets
        ...
        % En R2017, xline puede no existir, usar line en su lugar
        y_lim = ylim;
        line([mu mu], y_lim, 'LineStyle', '--', 'Color', colors(i,:));
    end
    ...
    for i = 1:size(M_clean,1)
        ...
        % En R2017, xline puede no existir, usar line en su lugar
        y_lim = ylim;
        line([mu mu], y_lim, 'LineStyle', '--', 'Color', colors(i,:));  % Mean line without label
    end
