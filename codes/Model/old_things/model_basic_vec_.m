...
function gas = dissociation(P, pars)
    ...
    %pars
    A1 = pars(41); % parameter in O2 dissociation equation
    A2 = pars(42); % parameter in CO3 dissociation equation
    alpha1 = pars(43); % parameter in O3 dissociation equation
    alpha2 = pars(44); % parameter in CO3 dissociation equation
    K1 = pars(39); % parameter in O3 dissociation equation
    K2 = pars(40); % parameter in CO3 dissociation equation
    beta1 = pars(45); % parameter in O3 dissociation equation
    beta2 = pars(46); % parameter in CO3 dissociation equation
    C1 = pars(47);
    C2 = pars(48);
    Z = pars(38);        
    CO2a_ = C2 * Z;
...
function [dv, dMRtgas, test_] = tissue(t, y, pars, test)
    ...
    Vtissue_CO2 = pars(20);
    Vtissue_O2 = pars(21);
    MRO2 = pars(54);   %This parameters correspond to the input values.  
    MRCO2 = pars(53); %This parameters correspond to the input values.
...
function [internal_variables_, test_] = vena_cava(t, y, pars, internal_variables, test)
    ...
    %vena_cava
        ...
        K_r_vc = pars(117);
        R_vc_n = pars(118);
        V_unstressed_vc = pars(119);
        V_vc_max = pars(120);
        V_vc_min = pars(121);
    
    %pulmonary
        V_unstressed_ra = pars(147);
        C_ra = pars(133);
...
function [internal_variables_,  dzheta_heart, dV_total_rv, dV_total_ra, dV_total_la, dV_total_lv, dQla, dTheart, test_] = heart(t, y, pars, internal_variables, test)
    ...
    %pars
        %heart
            ...
            R_ra = pars(142);
            Tsys_0 = pars(143);
            V_unstressed_la = pars(144);
            V_unstressed_lv = pars(145);
            V_unstressed_rv = pars(146);
            %for other modules
                %C_ra = pars(133);
                %V_unstressed_ra = pars(147);
        
        %pulmonary
            V_unstressed_pa = pars(129);
            C_pa = pars(122);
            V_unstressed_pv = pars(131);
            C_pv = pars(124);
            R_pv = pars(128);
...
function [internal_variables_, dV_total_vc, dV_total_v, dP_sp, dQbp, test_] = systemic_peripheric_and_venous_circulation(t, y, pars, internal_variables, test)
    ...
    %compartment order: e,s,b,h,rm,am
        %systemic_peripheric_and_venous_circulation    
            ...
            R_rm_n = pars(98);
            R_am_n = pars(99);
            V_unstressed_e_p = pars(100);
            V_unstressed_s_p = pars(101); 
            V_unstressed_b_p = pars(102);
            V_unstressed_h_p = pars(103);
            V_unstressed_rm_p = pars(104);
            V_unstressed_am_p = pars(105);
            %V_unstressed_e_v = pars(106);    %this is actually a var from carfiac control
            %V_unstressed_s_v = pars(107);    %this is actually a var from carfiac control
            V_unstressed_b_v = pars(108);
            V_unstressed_h_v = pars(109);
            %V_unstressed_rm_v = pars(110);  %this is actually a var from carfiac control
            %V_unstressed_am_v = pars(111);  %this is actually a var from carfiac control
            V_tot = pars(112);
...
function [dDThetaO2_s, dDThetaCO2_s, internal_variables_] = cns_ischemic_response(t, y, pars, internal_variables)
     ...
     k_isc_p_s = pars(203);
     k_isc_v_s = pars(204);
     PO2_ref_h_s = pars(205);
     PO2_ref_p_s = pars(206);
     PO2_ref_v_s = pars(207);
     tau_cc = pars(208);
     tau_isc = pars(209);
     Theta_h_s_n = pars(210);
     Theta_p_s_n = pars(211);
     Theta_v_s_n = pars(212);
     x_h_s = pars(213);
     x_p_s = pars(214);
     x_v_s = pars(215);
     PaCO2_n = pars(163);
...
function [internal_variables_] = efferent_pathways(t, y, pars, internal_variables)
    ...
    kcc_v_s = pars(230);
    kcc_v = pars(231);
    gamma_h_s_max = pars(232);
    gamma_p_s_max = pars(233);
    gamma_v_s_max = pars(234);
    gamma_v_max = pars(235);
    gamma_h_s_min = pars(236);
    gamma_p_s_min = pars(237);
    gamma_v_s_min = pars(238);
    gamma_v_min = pars(239);
    Theta_v = pars(240);
    Wb_h_s = pars(241);
    Wb_p_s = pars(242);
    Wb_v_s = pars(243);
...
function [dDTheta, dfh_s, dfp_s, dfv_s, internal_variables_] = reflex_control_R_Vu_E(t, y, pars, internal_variables, index_fun)
    ...
    %pars
    delay_Emax_lv = pars(256);
    delay_Emax_rv = pars(257);
    delay_R_am_p = pars(258);
    delay_R_e_p = pars(259);
    delay_R_rm_p = pars(260);
    delay_R_s_p = pars(261);
    delay_V_u_am_v = pars(262);
    delay_V_u_e_v = pars(263);
    delay_V_u_rm_v = pars(264);
    delay_V_u_s_v = pars(265);
    Emax_lv_0 = pars(266);
    Emax_rv_0 = pars(267);
    R_am_p_0 = pars(268);
    R_e_p_0 = pars(269);
    ...
    G_V_u_rm_v = pars(284);
    G_V_u_s_v = pars(285);
    tau_Emax_lv = pars(286);
    tau_Emax_rv = pars(287);
    tau_R_am_p = pars(288);
    tau_R_e_p = pars(289);
    tau_R_rm_p = pars(290);
    tau_R_s_p = pars(291);
    tau_V_u_am_v = pars(292);
    tau_V_u_e_v = pars(293);
    tau_V_u_rm_v = pars(294);
    tau_V_u_s_v = pars(295);
    fes_min = pars(296);
