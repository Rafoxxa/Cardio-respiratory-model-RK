...
function plot_masked_comparison(y_cells_N, t_cells_N, y_cells_H, t_cells_H, variables_to_show, variable_names, units_table, width, height, subject)
    ...
    for v = 1:num_vars
        ...
        Y_subject_N = Y_interp_N(subject,:);
        YN2plot = Y_subject_N(~isnan(Y_subject_N))
        t_grid_N = t_grid(~isnan(Y_subject_N));%t_grid_N = t_grid(~isnan(mean_N));
        ...
        Y_subject_H = Y_interp_H(subject,:);
        YH2plot = Y_subject_H(~isnan(Y_subject_H));
        t_grid_H = t_grid(~isnan(Y_subject_H)); %t_grid_H = t_grid(~isnan(mean_H));
        power_H = power_interp(~isnan(Y_subject_H));
        ...
        plot(t_grid_N, YN2plot, 'Color', color_N, 'LineWidth', 2, ...
             'DisplayName', 'Normoxia', 'LineStyle','-');
        ...
        plot(t_grid_H, YH2plot, 'Color', color_H, 'LineWidth', 2, ...
             'DisplayName', 'Hypoxia', 'LineStyle','-');
    
        yyaxis right        
        plot(t_grid_H, power_H, 'DisplayName', 'Power', 'Color', 'black');        
        ax = gca;
        ax.YColor = [0 0 0];
