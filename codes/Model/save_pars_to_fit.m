function save_pars_to_fit(patient, tag, pars_to_fit, folder)
   ...
   if ~strcmp(tag, '')
        ...
        path = sprintf('../Fitting/pars2fit/%d/%s/historic/', patient, tag);    
        if ~exist(path, 'dir')
            mkdir(path);
        end
    ...
    elseif strcmp(folder, 'last')  % Usar strcmp en lugar de ==
        last_path = sprintf('../Fitting/pars2fit/%d/%s/last/', patient, tag);
        hist_path = sprintf('../Fitting/pars2fit/%d/%s/historic/', patient, tag);
