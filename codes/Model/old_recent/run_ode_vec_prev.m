function [t, x_dot, x_vars, X_KEYS, INDEX] = run_ode(model, pars, init, taus, simulation_time, dt, control_on)
...
PARS = pars;   
    ...
    function [PARS_, stacked, cycle] = cycle_driver(stacked, cycle, DT, INDEX, PARS)       
        ...
        cycle.time_interval = cycle.t_ini: DT: cycle.t_end;
        cycle.progress = PARS(76)/SIMULATION_TIME * 100;
    ...
    function J = cost_fun(cycle, OPTIONS, PARS, args)
        ...
        optim_pars = PARS;
        disp(args)
        optim_pars(315) = args(1);
        optim_pars(316) = args(2);
        optim_pars(317) = 0;  %fixed by authors
        optim_pars(318) = args(3);
        optim_pars(319) = args(4);
        optim_pars(320) = args(5);
        
        optim_pars(321) = optim_pars(315) + optim_pars(316);
        ...
        function J = respiratory_work(state_vars, OPTIONS, optim_pars)
            ...
            Pmax = optim_pars(74);
            dPmax = optim_pars(75);
            lambda1 = optim_pars(72);
            lambda2 = optim_pars(73);
            n = optim_pars(71);
            DT = optim_pars(2)*10;
    ...
    function args_optimal = control_optimization(cycle, OPTIONS, PARS, dVE)
        ...
        lb = [pars(305), pars(306), pars(307), pars(308), pars(309)];
        ub = [pars(310), pars(311), pars(312), pars(313), pars(314)];
        disp(lb)
        disp(ub)
        disp("____")
        ...
        cost_fun(cycle, OPTIONS, PARS, args_init)
    ...
    function status = simple_output_fcn(t, y, flag, pars, varargin)
        ...
        elseif isempty(flag)
            ...
            if delta_step > 0
                all_global(1,iter_step_prev_plus_1:iter_step_plus_1) = y(13);
                all_global(2,iter_step_prev_plus_1:iter_step_plus_1) = y(52);
                all_global(3,iter_step_prev_plus_1:iter_step_plus_1) = y(51);
                all_global(4,iter_step_prev_plus_1:iter_step_plus_1) = y(15);
                all_global(5,iter_step_prev_plus_1:iter_step_plus_1) = y(93);
                all_global(6,iter_step_prev_plus_1:iter_step_plus_1) = y(95);
                all_global(7,iter_step_prev_plus_1:iter_step_plus_1) = y(92);
                all_global(8,iter_step_prev_plus_1:iter_step_plus_1) = y(127);
                all_global(9,iter_step_prev_plus_1:iter_step_plus_1) = y(128);
                all_global(10,iter_step_prev_plus_1:iter_step_plus_1) = y(129);
                all_global(11,iter_step_prev_plus_1:iter_step_plus_1) = y(130);
                all_global(12,iter_step_prev_plus_1:iter_step_plus_1) = y(131);
            
            elseif delta_step <= 0

                all_global(1,iter_step_plus_1) = y(13);
                all_global(2,iter_step_plus_1) = y(52);
                all_global(3,iter_step_plus_1) = y(51);
                all_global(4,iter_step_plus_1) = y(15);
                all_global(5,iter_step_plus_1) = y(93);
                all_global(6,iter_step_plus_1) = y(95);
                all_global(7,iter_step_plus_1) = y(92);
                all_global(8,iter_step_plus_1) = y(127);
                all_global(9,iter_step_plus_1) = y(128);
                all_global(10,iter_step_plus_1) = y(129);
                all_global(11,iter_step_plus_1) = y(130);
                all_global(12,iter_step_plus_1) = y(131);
