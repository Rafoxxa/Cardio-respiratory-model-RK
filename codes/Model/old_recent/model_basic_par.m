...
function xdot= model_basic_par(t, y, pars, init_keys)
 ...
 %% Testing variables: internal variables and other non state variables sometimes needs to be plotted, for that we create forced variables to stored them in the result's vector    
  ...
  %% Physiology Equations
    ...
    kkeys = vertcat(internal_vars_muscle_pump.keys,  internal_vars_pc.keys);
    vvalues = vertcat(internal_vars_muscle_pump.values,  internal_vars_pc.values);
    internal_vars_sys_periph_ = dictionary(kkeys, vvalues);
    ...
    kkeys = vertcat(internal_vars_heart.keys, internal_vars_sys_periph.keys);
    vvalues = vertcat(internal_vars_heart.values, internal_vars_sys_periph.values);
    internal_vars_coronary_resting_ = dictionary(kkeys, vvalues);
    kkeys = vertcat(internal_vars_sys_periph.keys,  internal_vars_met.keys);
    vvalues = vertcat(internal_vars_sys_periph.values,  internal_vars_met.values);
    internal_vars_active_blood_ = dictionary(kkeys, vvalues);    
    ...
    kkeys = vertcat(internal_vars_cns.keys,  internal_vars_met.keys, internal_vars_neuro_drive.keys, internal_vars_aff_baro.keys);
    vvalues = vertcat(internal_vars_cns.values,  internal_vars_met.values, internal_vars_neuro_drive.values, internal_vars_aff_baro.values);
    internal_vars_efferent_ = dictionary(kkeys, vvalues);
...
function  [ddVE, all_global_vent] = ventilation_control(t, y, pars, internal_variables, all_global_lol)
    ...
    ddVE = (dVE - y("dVE"))/dt;
    all_global_lol(index(tiny_y_keys, 'dVE'), round(t/dt) + 1) = dVE;
