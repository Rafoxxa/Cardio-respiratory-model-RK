...
function xdot= model_basic_par(t, y, pars, init_keys)
    time = t;
    init_values = y;   
    pars_lol = pars; 
    global all_global;
 ...
 %% Testing variables: internal variables and other non state variables sometimes needs to be plotted, for that we create forced variables to stored them in the result's vector    
  ...
  %% Physiology Equations
    ...
    % Initial parallel executions of independent tasks
    f_muscle_pump = parfeval(pool, @muscle_pump, 2, t, y, pars, internal_variables, test);
    f_ventilation_control = parfeval(pool, @ventilation_control, 2, t, y, pars, internal_variables, all_global);
    f_resp_pump = parfeval(pool, @respiratory_pump, 2, t, y, pars, internal_variables, test);
    f_upper_airways = parfeval(pool, @upper_airways, 6, t, y, pars, internal_variables, test);
    f_metabolic_reg = parfeval(pool, @metabolic_regulation, 3, t, y, pars, internal_variables, test);   
    f_exchange_mixing = parfeval(pool, @exchange_mixing, 10, t, y, pars, index, test);
    f_brain = parfeval(pool, @brain, 4, t, y, pars, internal_variables, test);
    f_metabolism_dyn = parfeval(pool, @metabolism_dynamics, 4, t, y, pars, test);
    f_afferent_baro = parfeval(pool, @afferent_barorreflex, 1, t, y, pars, internal_variables);
    f_afferent_chemo = parfeval(pool, @afferent_chemoreceptor, 1, t, y, internal_variables);
    f_afferent_stretch = parfeval(pool, @afferent_pulmonary_stretch, 1, t, y, pars, internal_variables);
    f_cns_isquemic_response = parfeval(pool, @cns_ischemic_response, 3, t, y, pars, internal_variables);

    % Wait for results and start dependent tasks
    [y_ua, ddVua, dGaw, dVua, internal_variables_upper, test] = fetchOutputs(f_upper_airways);
    f_pulmonary_mechanics = parfeval(pool, @pulmonary_mechanics, 6, t, y_ua, pars, internal_variables_upper, test);

    [dPvbCO2, dPCSFCO2, internal_vars_brain, test_brain] = fetchOutputs(f_brain);
    f_mean_values = parfeval(pool, @mean_values_computer, 5, t, y, pars, internal_vars_brain, test_brain);

    [internal_vars_resp_pump, test_resp_pump] = fetchOutputs(f_resp_pump);
    f_vena_cava = parfeval(pool, @vena_cava, 2, t, y, pars, internal_vars_resp_pump, test);
    
    [internal_vars_vc, test_vc] = fetchOutputs(f_vena_cava);
    f_heart = parfeval(pool, @heart, 9, t, y, pars, internal_vars_vc, test);
    
    [internal_vars_heart, dzheta_heart, dV_total_rv, dV_total_ra, dV_total_la, dV_total_lv, dQla, dTheart, test] = fetchOutputs(f_heart);
    f_systemic_arteries = parfeval(pool, @systemic_arteries, 5, t, y, pars, internal_vars_heart, test);
    f_pulmonary_circulation = parfeval(pool, @pulmonary_circulation, 7, t, y, pars, internal_vars_heart, test);
    
    [internal_vars_pc, dQ_pa, dV_total_pp, dV_total_pv, dV_total_pa, dQpp, test] = fetchOutputs(f_pulmonary_circulation);    
    [internal_vars_muscle_pump, test_muscle_pump] = fetchOutputs(f_muscle_pump);
    
    internal_vars_sys_periph_ = internal_vars_muscle_pump.* (internal_vars_muscle_pump ~= internal_vars_pc) + internal_vars_pc;
    f_systemic_peripheric = parfeval(pool, @systemic_peripheric_and_venous_circulation, 6, t, y, pars, internal_vars_sys_periph_, test);
    
    [I, internal_vars_met, test_met] = fetchOutputs(f_metabolic_reg);    
    [internal_vars_sys_periph, dV_total_vc, dV_total_v, dP_sp, dQbp, test_sys_periph] = fetchOutputs(f_systemic_peripheric);
    [y_ex, dPAgas, ddPa, dP_1, dP_2, dP_3, dP_4, dP_5, a, ~] = fetchOutputs(f_exchange_mixing);
    internal_vars_coronary_resting_ = internal_vars_heart.* (internal_vars_heart ~= internal_vars_sys_periph) + internal_vars_sys_periph;
    internal_vars_active_blood_ = internal_vars_sys_periph.* (internal_vars_sys_periph ~= internal_vars_met) + internal_vars_met;
    f_tissue = parfeval(pool, @tissue, 3, t, y_ex, pars, test);
    f_cerebral_blood_flow = parfeval(pool, @cerebral_blood_flow, 4, t, y_ex, pars, internal_variables);
    f_coronary_resting_muscle = parfeval(pool, @coronary_and_resting_muscle_blood_flow, 4, t, y_ex, pars, internal_vars_coronary_resting_);
    f_active_muscle_flow = parfeval(pool, @active_muscle_blood_flow, 5, t, y_ex, pars, internal_vars_active_blood_, index_fun);

    [ddVE, all_global_vent] = fetchOutputs(f_ventilation_control);
    f_neuromuscular_drive = parfeval(pool, @neuromuscular_drive, 3, t, y, pars, internal_variables, index, tiny_y_keys, all_global_vent);
    
    [~, internal_vars_neuro_drive, test_neuro_drive] = fetchOutputs(f_neuromuscular_drive);
    [dDThetaO2_s, dDThetaCO2_s, internal_vars_cns] = fetchOutputs(f_cns_isquemic_response);
    [internal_vars_aff_baro] = fetchOutputs(f_afferent_baro);  
    internal_vars_efferent_ = internal_vars_cns .* (internal_vars_cns ~= internal_vars_met) + internal_vars_met;
    internal_vars_efferent_ = internal_vars_aff_baro .* (internal_vars_aff_baro ~= internal_vars_efferent_) + internal_vars_efferent_;
    internal_vars_efferent_ = internal_vars_neuro_drive .* (internal_vars_neuro_drive ~= internal_vars_efferent_) + internal_vars_efferent_;
    f_efferent_pathways = parfeval(pool, @efferent_pathways, 1, t, y, pars, internal_vars_efferent_ );

    internal_vars_efferent = fetchOutputs(f_efferent_pathways);
    f_reflex_control_R_V = parfeval(pool, @reflex_control_R_Vu_E, 5, t, y, pars, internal_vars_efferent, index_fun);
    f_reflex_control_HR = parfeval(pool, @reflex_control_HR, 3, t, y, pars, internal_vars_efferent, index_fun);

    %% Fetch remaining outputs
    
    [~, dPmusc, dPpl, dV, ddV, test] = fetchOutputs(f_pulmonary_mechanics);
    [dv, dMRtgas, ~] = fetchOutputs(f_tissue);
    [dxO2_b, dxCO2_b, dR_bp, ~] = fetchOutputs(f_cerebral_blood_flow);
    [dxO2, dxCO2, dWh, ~] = fetchOutputs(f_coronary_resting_muscle);
    [dxO2_am, dx_met, dx_M, dphi_met, ~] = fetchOutputs(f_active_muscle_flow);
    [dMRR, dM_Rv, dMRv, ~] = fetchOutputs(f_metabolism_dyn);
    [mean_dPaO2, mean_dPaCO2, mean_dPbCO2, mean_dP_sa, ~] = fetchOutputs(f_mean_values);
    [~, dP_sa, dQ_sa, dP_mean, ~] = fetchOutputs(f_systemic_arteries);
    [dDTheta, dfh_s, dfp_s, dfv_s, ~] = fetchOutputs(f_reflex_control_R_V);
    [dDTsym, dDTvagal, dfv] = fetchOutputs(f_reflex_control_HR);
    [df_ac] = fetchOutputs(f_afferent_chemo);
    [df_ap] = fetchOutputs(f_afferent_stretch);
  ...
  %% dxdt building   
    ...
    %diff = xdot_dict.values - xdot_dict_.values;
    %keys = xdot_dict.keys;
    %disp(keys(diff ~= 0));
    %disp(diff(diff ~= 0));
...
function [y_, internal_variables_, test_] = neuromuscular_drive(t, y, pars, internal_variables, index, tiny_y_keys, all_global)
    ...
    internal_variables(89) = Nt;  %dejar derivada en 0 y guardar todas las variables externas en una 
    internal_variables_ = internal_variables; 
    y_ = y;
    test_ = test;
...
function  [ddVE, all_global_vent] = ventilation_control(t, y, pars, internal_variables, all_global_lol)
    ...
    ddVE = (dVE - y(13))/dt;
    all_global_lol(1, round(t/dt) + 1) = dVE;
    all_global_vent = all_global_lol;
...
function [internal_variables_, test_] = vena_cava(t, y, pars, internal_variables, test)
    ...
    %Saving computations
        ...
        internal_variables(55) = P_vc;
        internal_variables(52) = V_vc;
        internal_variables(44) = Q_ra;
        internal_variables(19) = P_ra;
        internal_variables(45) = V_ra;
        internal_variables_ = internal_variables;
    test_ = test;
end
...
function [internal_variables_,  dzheta_heart, dV_total_rv, dV_total_ra, dV_total_la, dV_total_lv, dQla, dTheart, test_] = heart(t, y, pars, internal_variables, test)
    ...
    %------------------------------------
    %internal_variables
    Theart = internal_variables(122);
    Ptor = internal_variables(35);  
    Q_ra = internal_variables(44);
    P_ra = internal_variables(19);
    E_max_lv = internal_variables(120);
    E_max_rv = internal_variables(121);
    if t == 0
        E_max_lv = y(89);
        E_max_rv = y(90);
    end
    ...
    % Saving computations
        ...
        internal_variables(49) = V_pa;
        internal_variables(51) = V_pv;
        internal_variables(47) = V_la;
        internal_variables(48) = V_lv;
        internal_variables(46) = V_rv;
        internal_variables(74) = Wh_lv;
        internal_variables(75) = Wh_rv;
        internal_variables_ = internal_variables;
...
function [internal_variables_, dP_sa, dQ_sa, dP_mean, test_] = systemic_arteries(t, y, pars, internal_variables, test)
...
function [internal_variables_, dV_total_vc, dV_total_v, dP_sp, dQbp, test_] = systemic_peripheric_and_venous_circulation(t, y, pars, internal_variables, test)  
    ...
    Pabd = internal_variables(43);
    Q_ra = internal_variables(44);
    V_ra = internal_variables(45); 
    V_rv = internal_variables(46);
    V_la = internal_variables(47);
    V_lv = internal_variables(48);
    V_pa = internal_variables(49);
    V_pp = internal_variables(50); 
    V_pv = internal_variables(51); 
    V_vc = internal_variables(52); 
    %V_sa = internal_variables(53); 
    P_im = internal_variables(54); 
    P_vc = internal_variables(55); 

    R_e_p = internal_variables(112);
    R_s_p = internal_variables(113);
    R_b_p = internal_variables(126);
    R_h_p = internal_variables(124);
    R_rm_p = internal_variables(125);
    R_am_p = internal_variables(123);

    V_unstressed_e_v = internal_variables(116);
    V_unstressed_s_v = internal_variables(117);
    V_unstressed_rm_v = internal_variables(118);
    V_unstressed_am_v = internal_variables(119);
    ...
    %Saving computations    
    internal_variables(81) = Q_p(6);
    internal_variables(76) = Q_p(4);
    internal_variables(77) = Q_p(5);
    internal_variables(71) = Q_p(3);
    internal_variables_ = internal_variables;
    test_ = test;
end

function [internal_variables_] = afferent_barorreflex(t,y,pars, internal_variables)
...
function  [dxO2, dxCO2, dWh, internal_variables_] = coronary_and_resting_muscle_blood_flow(t, y, pars, internal_variables)
    ...
    R_rm_p_n = internal_variables(114);   
    tau_w = pars(187);   
    tau_O2 = pars(177);
    tau_CO2 = pars(176);
    Whn = pars(188); 
    ...
    %internal variables
    Wh_lv = internal_variables(74);
    Wh_rv = internal_variables(75);
    Q_h_p = internal_variables(76);
    Q_rm_p = internal_variables(77);
    Q_p = [Q_h_p, Q_rm_p];
    ...
    internal_variables(124) = Rp(1);
    internal_variables(125) = Rp(2);
    internal_variables_ = internal_variables;
...
function [dxO2_am, dx_met, dx_M, dphi_met, internal_variables_] = active_muscle_blood_flow(t,y,pars, internal_variables, index_fun)
    ...
    %internal variables
    I = internal_variables(87);
    Q_am_p = internal_variables(81);
    %Equations
    R_amp_n = internal_variables(115);
...
function [dDThetaO2_s, dDThetaCO2_s, internal_variables_] = cns_ischemic_response(t, y, pars, internal_variables)
     ...
     internal_variables(90) = Theta_s(1);
     internal_variables(91) = Theta_s(2);
     internal_variables(92) = Theta_s(3);
...
function [internal_variables_] = efferent_pathways(t, y, pars, internal_variables)
    ...
    %internal variables
    I = internal_variables(87);
    fab = internal_variables(88);
    Nt = internal_variables(89);    
    
    Theta_h_s = internal_variables(90);
    Theta_p_s = internal_variables(91);
    Theta_v_s = internal_variables(92);
    ...
    %internal variables
    internal_variables(110) = fs(1);
    internal_variables(98) = fs(2);
    internal_variables(99) = fs(3);
    internal_variables(111) = fv;
...
function [dDTheta, dfh_s, dfp_s, dfv_s, internal_variables_] = reflex_control_R_Vu_E(t, y, pars, internal_variables, index_fun)
    ...
    %internal variables
    f_h_s_actual = internal_variables(110);  
    f_p_s_actual = internal_variables(98);
    f_v_s_actual = internal_variables(99);
    ...
    internal_variables(112) = Theta(1);
    internal_variables(113) = Theta(2);
    internal_variables(114) = Theta(3);
    internal_variables(115) = Theta(4);
    internal_variables(116) = Theta(5);
    internal_variables(117) = Theta(6);
    internal_variables(118) = Theta(7);
    internal_variables(119) = Theta(8);
    internal_variables(120) = Theta(9);
    internal_variables(121) = Theta(10);
...
function [dDTsym, dDTvagal, dfv, internal_variables_] = reflex_control_HR(t, y, pars, internal_variables, index_fun)
    ...
    %internal variables
    f_h_s_actual = internal_variables(110);
    f_v_actual = internal_variables(111);
...
function [internal_variables_] = adding_base_values_in_control_variables(t, y, pars, internal_variables)
    ...
    internal_variables(120) = Theta(9);
    internal_variables(121) = Theta(10);
    internal_variables(122) = Theart;    
    internal_variables(123) = R_am_p;
    internal_variables(124) = Rp(1);
    internal_variables(125) = Rp(2);
    internal_variables(126) = R_bp;
...
function [all_global_, tiny_y_keys] = saving_in_globals(all_global, y)
    ...
    all_global(:, round(time/dt) + 1) = arr';
