...
general_structures.hipoxia = hipoxia_state;
...
%for i = 1:4
    %if i > 0 
        ...
        for i = 1:size(general_structures.cpet_filenames,2)   
            ...
            X_cpet.AT = AT;
            X_cpet.ladder_points = ladder_points;
            
            %ladder_points is pendant;
            

            data_structures.X = X_cpet;
        ...
        general_structures = align_and_resample(general_structures);
...
function general_structures = setting_up_general_structures(general_structures)
    ...
    if ga.hipoxia
        ...
        finapres_not_nan_intevals4 = [35.65 * 60, -1];
        cpet_filenames4 = ["../../data/HIPOXIA/CV/ASCENSO/CPET_HIPOXIA_ASCENSO_CV.xlsx", "../../data/HIPOXIA/VC/NEW_CPET_HIPOXIA_EJERCICIO_VC.xlsx"];
        ...
        finapres_not_nan_intevals1 = [39.86 * 60, -1]; 
...
function general_structures = align_and_resample(general_structures)
...
%     4. Si vars2 tiene NaN fuera del rango, se extienden los valores más cercanos.
    ...
    finapres_not_nan_mask = ones(size(vars2_new));
    if ~isempty(f_n_i)
        init_ = f_n_i(1);
        end_  = f_n_i(end);
